...
grouping strongly co-expressed genes together. It could be groups of genes 
...
you'll need to do is download the .gmt files using the MSigDB page.
...
```{r, echo=FALSE, eval=FALSE}
save(msigdb.enrichment, file = "../mixtData/data/msigdb.enrichment.RData")
```

```{r, echo=FALSE}
load(url("https://rawgit.com/vdumeaux/mixtData/master/data/msigdb.enrichment.RData"))
```
  
Again, for the web application data must be saved in the data folder of
the mixt package
```{r, eval=FALSE}
devtools::use_data(msigdb.enrichment, overwrite = TRUE)
```
...
```
```{r, echo=FALSE, eval=FALSE}
save(perm.cor.p, file="../mixtData/data/perm.cor.p.RData")
```
```{r, echo=FALSE}
load(url(("https://rawgit.com/vdumeaux/mixtData/master/data/perm.cor.p.RData")))
```
```{r}
str(perm.cor.p)
head(perm.cor.p$all)
```
```{r, eval=FALSE}
devtools::use_data(perm.cor.p, overwrite = TRUE)
```
