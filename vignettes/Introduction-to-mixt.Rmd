...
* `cohorts` list of patients subgroups. If you have only one name, the vector
...
```{r}
epi.clinical.assoc.ern <- clinical.ranksum(mixt.dat = dat, 
                                           mixt.ranksum = bresat, 
                                           tissue="epi", cohort="ern")
knitr::kable(epi.clinical.assoc.ern[, 1:6])

stroma.clinical.assoc.ern <- clinical.ranksum(mixt.dat = dat, 
                                              mixt.ranksum = bresat,
                                              tissue="stroma", cohort="ern")
...
```{r, eval=FALSE}
perm.cor.p <- stat.ranksum (mixt.ranksum = bresat, nRuns = 10000, mc.cores = 2)
```
```{r, echo=FALSE, eval=FALSE}
save(perm.cor.p, file="../mixtData/data/perm_cor_p.RData")
```
```{r, echo=FALSE}
...
cohort_scatterplot(mixt.ranksum = bresat, mixt.stat = perm.cor.p, 
