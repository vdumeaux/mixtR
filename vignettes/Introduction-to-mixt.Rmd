...
fdr <- NULL
fdr <- lapply(emp.p, function(p.dat){
...
```{r, echo=FALSE, eval=FALSE}
save(fdr, file="../mixtData/data/mod_clinical_fdr.RData")
```
...
clinicalVars = rownames(epi.clinical.assoc.ern)
cols = colnames(epi.clinical.assoc.ern)
epi.clinical.assoc.ern = cbind(clinicalVars,epi.clinical.assoc.ern)
colnames(epi.clinical.assoc.ern) = c("Clinical", cols)
```

```{r, echo=FALSE, eval=FALSE}
save(perm_cor_p, file="../mixtData/data/mod_clinical_fdr.RData")
```
...
modules. P-values are adjusted for multiple testing using the false discovery 
rate of (Benjamini and Hochberg, 1995). 

```{r}
gene.set.overlap <-  WGCNA::overlapTable(moduleColors[[1]], moduleColors[[2]])
str(gene.set.overlap)
knitr::kable(gene.set.overlap$pTable[1:10, 1:8])
```

This output is used by the D3 Javascript library (http://d3js.org) to build 
interactive visualization in the mixt web application.  
